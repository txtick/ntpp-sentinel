...
- **Google Pollen does not provide historical lookup by past date.** When `POLLEN_PROVIDER=google`, the scheduled pollen snapshot stores Google's rolling forecast entries, up to `GOOGLE_POLLEN_FORECAST_DAYS` dates (`5` max). This makes upcoming dates available before anyone opens the dashboard and later turns those stored forecast rows into dashboard history.
...
- **The homepage treats weather as best-effort.** Summary, alerts, and reminders render first; the weather card fills in after `/api/weather` returns so a slow external weather or pollen provider does not hold the whole dashboard page load.
- **Weather/pollen external call timeouts are intentionally short on dashboard loads.** Tune `WEATHER_API_TIMEOUT`, `WEATHER_AQ_TIMEOUT`, and `POLLEN_API_TIMEOUT` if providers get flaky. Live dashboard weather uses one pollen attempt; the scheduled pollen snapshot job is the more reliable backfill path.
- **Pollen history was previously traffic-driven.** Before the dedicated `weather_pollen` cron job and Google forecast-date upserts, blank days appeared whenever `/api/weather` was not loaded that day or the live pollen fetch failed.
...
- Skimmer marks a route assignment deleted when a customer is made inactive. Customer-flow history must retain those deleted assignments and inactive customer rows, while treating only non-deleted assignments as current; filtering them out makes real losses disappear from the widget.
