    ...
    with conn.cursor() as cur:
        ...
        cur.execute(
            ...
            SELECT
                ...
                (SELECT COUNT(DISTINCT customer_id) FROM alert_instances WHERE status NOT IN ('cleared', 'resolved') AND category = 'pool') AS customers_with_current_alerts,
                (SELECT COUNT(*) FROM alert_instances WHERE status NOT IN ('cleared', 'resolved') AND severity = 'critical') AS critical_current_alert_count,
                (SELECT COUNT(*) FROM alert_instances WHERE status NOT IN ('cleared', 'resolved') AND category = 'process') AS chemistry_trend_alert_count,
                (SELECT COUNT(*) FROM alert_instances WHERE status NOT IN ('cleared', 'resolved') AND category = 'revenue') AS revenue_opportunity_count
            """
        )
