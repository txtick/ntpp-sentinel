...
CUSTOMER_SYNC_AFTER_INGEST_ENABLED=1
CUSTOMER_SYNC_AFTER_INGEST_LIMIT=1000
```

`INGEST_WORKER_SECRET` may be left blank if you want it to reuse `WEBHOOK_SECRET`.
`CUSTOMER_SYNC_SECRET` may also be left blank if you want it to reuse `WEBHOOK_SECRET`.
